...
[ $# -eq 1 ] || { echo "usage: $0 [--rotate] [--reviewer LOGIN] OWNER/REPO" >&2; exit 2; }
REPO="$1"
ENV_NAME="release"
...
JSON
if ! gh api "repos/$REPO/environments/$ENV_NAME/deployment-branch-policies" -q '.branch_policies[].name' | grep -qx main; then
...
existing=$(gh api "repos/$REPO/rulesets" -q ".[] | select(.name == \"$RULESET_NAME\") | .id" | head -1)
if [ -z "$existing" ]; then
...
JSON
  ...
  if gh api "repos/$REPO/rulesets/$existing" -q '.bypass_actors[].actor_type' | grep -qx DeployKey; then
...
key_id=$(gh repo deploy-key list --repo "$REPO" --json id,title -q ".[] | select(.title == \"$KEY_TITLE\") | .id" | head -1)
secret_present=false
if gh api "repos/$REPO/environments/$ENV_NAME/secrets" -q '.secrets[].name' | grep -qx "$SECRET_NAME"; then
  secret_present=true
fi
